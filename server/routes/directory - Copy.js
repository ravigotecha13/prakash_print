...
router.delete('/:id', UserCtrl.authMiddleware, function(req, res) {
  const user = res.locals.user;
console.log('221121221');
console.log(req.params.id);
Direcotry.findOneAndRemove({'city': req.params.id}, function(err){
     if (err) {
      return res.status(422).send({errors: normalizeErrors(err.errors)});
    }
...
router.post('/csvupload', multipartMiddleware, function(req, res) {
 ...
 for(var i =0; i<xlData.length;i++){
  ...
  var category = splitcscdata.Category.toLowerCase();
    if(splitcscdata[0] != ''){
      ...
      console.log(splitcscdata.Category.toLowerCase());
      Category.findOne({'name':splitcscdata.Category.toLowerCase()})
        .exec(function(err,foundCategory) {
           console.log(foundCategory);
console.log(err);
          if(foundCategory!=null){
console.log(k);
        var splitcscdata = xlData[k];
...
console.log(query);
  Direcotry.find({'category': req.body.category,'city' : req.body.city, 'status' : {"$nin": false}  }).populate('city category').exec(function(err,foundDirecotry) {
