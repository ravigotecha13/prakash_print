...
//      pressnote.size=req.body.size;
  ...
  Pressnote.create(pressnote, function(err, newPressnote) {
    ...
    if (err) {
     ...
     return res.json({error:false,message:'Historical data added Successfully',data:newPressnote});
  });
});
...
console.log(query);
   Pressnote.count({'type':{'$in':['history']}}).exec(function(err,totalCount){

    Pressnote.find({'type':{'$in':['history']}}).sort(query.sort).limit(query.limit).skip(query.skip).then(function(foundPressnotes) {
//    Pressnote.find({},,query,function(err, foundPressnotes) {

        if (city && foundPressnotes.length === 0) {
          return res.status(422).send({errors: [{title: 'No Historical Knowledge Found!', detail: `There are no pressnotes for city ${city}`}]});
        }
         var totalPages = Math.ceil(totalCount / size)
        return res.json({error:false,message:'Historical Knowledge Found Successfully',data:foundPressnotes,pages:totalPages});
      });
    });
});
